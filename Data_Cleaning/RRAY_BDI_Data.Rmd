...
Script for aggregating longitudinal BDI measures.

```{r setup, include=FALSE}
knitr::opts_chunk$set(echo = TRUE)
```

Start with raw qualtrics/redcap data.
```{r}
# 3 qualtrics surveys:
...
# make dates be consistent format
...
# compute days since baseline for the timing of each BDI measure
...
Compare what I have with previously scored BDI data from drive, check what's missing in each...
```{r}
...
# Start with old data, but add in new entries
...
# check no one has more timepoints than expected
bdi_count$participant[bdi_count$n_timepoints==10] # none


# check who has 2 or fewer timepoints
...
Write out file without empty rows for missing timepoints.
```{r}
write.csv(all_bdi_dem3, paste0("../../Data/Cleaned_Survey_Data/RRAY_All_BDI_", Sys.Date(), ".csv"),row.names = FALSE)
```

For Mplus, need empty rows for missing timepoints.
```{r}
#dim(depx_old)
dim(all_bdi_dem3)
...
length(unique(depx_NAs2$participant)) #n = 179

# write out file for mplus
