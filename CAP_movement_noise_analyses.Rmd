...
```{r}
write.csv(mydata, paste0("../Data/Brain_Data/Movement_",Sys.Date(),".csv"),row.names = FALSE)
```


```{r}
setwd("../Data/Movement2")
...
mydata2 = Reduce(function(...)merge(...,all=T),alldata)
```


```{r}
write.csv(mydata2, paste0("../Data/Brain_Data/Movement2_",Sys.Date(),".csv"),row.names = FALSE)
```
...
blong2 = merge(blong, mydata2, all.x = TRUE)
dim(blong2)
```
...
# averages for times when state is absent...
...
# do separately for state 8 since not everyone has it I guess...
...
rois_table = aggregate(list(rois$snr_avg), list(rois$state), mean, na.rm=T)
rois_table


# make my own codes...
rois$code1 = ifelse(rois$state==8, 7, -1)
rois$code2 = ifelse(rois$state==8, 0, ifelse(rois$state==7, 6, -1))
rois$code3 = ifelse(rois$state>=7, 0, ifelse(rois$state==6, 5, -1))
rois$code4 = ifelse(rois$state>=6, 0, ifelse(rois$state==5, 4, -1))
rois$code5 = ifelse(rois$state>=5, 0, ifelse(rois$state==4, 3, -1))
rois$code6 = ifelse(rois$state>=4, 0, ifelse(rois$state==3, 2, -1))
rois$code7 = ifelse(rois$state>=3, 0, ifelse(rois$state==2, 1, -1))


summary(lm(snr_avg ~ code1 + code2 + code3 + code4 + code5 + code6 + code7, data=rois))

```

```{r}
#only significant difference is state 8.

# helmert codes
# 7 -1 ... -1; state 8 vs others
# 0  6 ... -1; state 7 vs 6 and less
# 0  0 5 ...

# another approach, t-tests when state is present vs not:
# rearrange dataset again...
...
# 5 is only one that survives and is not negative...
