...
# Also track unidirectional transitions:
# transitions into task blocks
dsmooth$task_entries = 0 # default no transition = 0
allevents = length(dsmooth$voleventbin)
for (evnt in 2:allevents) {
  ev2 = dsmooth$voleventbin[evnt]
  ev1 = dsmooth$voleventbin[evnt-1]
...
# compute distance from previous transition:
translist = dsmooth$volnum[which(dsmooth$task_entries == 1)]
translist = c(translist, 720)  
    for (i in 1:(length(translist)-1)) {
      start = translist[i] # volume of transition
      stop = translist[i+1] # volume of next transition
      dsmooth$task_entries_dist[dsmooth$volnum %in% start:(stop-1)] = 0:(stop-start-1)
    }

# transitions into fix blocks
dsmooth$fix_entries = 0 # default no transition = 0
allevents = length(dsmooth$voleventbin)
for (evnt in 2:allevents) {
  ev2 = dsmooth$voleventbin[evnt]
  ev1 = dsmooth$voleventbin[evnt-1]
...
# compute distance from previous transition:
translist = dsmooth$volnum[which(dsmooth$fix_entries == 1)]
translist = c(translist, 720)  
    for (i in 1:(length(translist)-1)) {
      start = translist[i] # volume of transition
      stop = translist[i+1] # volume of next transition
      dsmooth$fix_entries_dist[dsmooth$volnum %in% start:(stop-1)] = 0:(stop-start-1)
    }
